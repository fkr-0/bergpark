...
start = source.index("  function activateTerrain() {\n")
end = source.index("\n  element.dataset.terrainEncoding", start)
replacement = r'''  function clearTerrainVerifyTimer() {
...
e2e = Path('tests/e2e/terrain-relief-regression.spec.js')
e2e_source = e2e.read_text()
old = """  await expect(map).toHaveAttribute('data-spatial-terrain-state', 'terrain');
  await expect(map).toHaveAttribute('data-spatial-terrain-ready', 'true', { timeout: 20_000 });
"""
new = """  await expect(map).toHaveAttribute('data-spatial-terrain-state', 'terrain');
  await expect(map).toHaveAttribute('data-spatial-terrain-active', 'true');
  await expect(map).toHaveAttribute('data-spatial-terrain-ready', 'true', { timeout: 20_000 });
"""
if e2e_source.count(old) != 1:
    raise SystemExit('unexpected terrain E2E shape')
e2e.write_text(e2e_source.replace(old, new, 1))

Path('tests/terrain-runtime-lifecycle.test.mjs').write_text(r'''import assert from 'node:assert/strict';
